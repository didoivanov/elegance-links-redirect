(function ($) {
	...
	var COUNTRIES = [
		...
		{ code: 'AX', name: 'Aland Islands' }, { code: 'AZ', name: 'Azerbaijan' },
		...
		{ code: 'BJ', name: 'Benin' }, { code: 'BL', name: 'Saint Barthelemy' },
		...
		{ code: 'CC', name: 'Cocos (Keeling) Islands' }, { code: 'CD', name: 'Congo (Kinshasa)' },
		{ code: 'CF', name: 'Central African Republic' }, { code: 'CG', name: 'Congo (Brazzaville)' },
		...
		{ code: 'CW', name: 'Curacao' }, { code: 'CX', name: 'Christmas Island' },
		...
		{ code: 'QA', name: 'Qatar' }, { code: 'RE', name: 'Reunion' },
	...
	var DEVICES = ['desktop', 'mobile', 'tablet', 'bot'];
	...
	function replaceLastTerm(value, replacement) {
		var parts = value.split(/,\s*/);
		parts.pop();
	...
	$(function () {
		var $input = $('#elr-match-value');
		var $type  = $('#elr-rule-type');
		var $hint  = $('.elr-match-hint');

		if (!$input.length || !$type.length || typeof $.fn.autocomplete !== 'function') {
			return;
		}

		function currentMode() {
			return $type.val() === 'device' ? 'device' : 'country';
		}

		function refreshHint() {
			var key = currentMode() === 'device' ? 'device-hint' : 'country-hint';
			$hint.text($hint.data(key) || '');
		}
		...
		$input.autocomplete({
			...
			select: function (event, ui) {
				var next = replaceLastTerm(this.value, ui.item.value) + ', ';
				this.value = next;
				return false;
			}
		});
